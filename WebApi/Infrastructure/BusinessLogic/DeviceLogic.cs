...
using PnIotPoc.WebApi.Common.Factory;
...
namespace PnIotPoc.WebApi.Infrastructure.BusinessLogic
{
    public class DeviceLogic : IDeviceLogic
    {
           ...
           DeviceModel device)
        {
            ...
            var propValModels = ExtractPropertyValueModels(deviceProperties);
            var hostNameValue = _configProvider.GetConfigurationSettingValue("iotHub.HostName");
        ...
        private static void ApplyPropertyValueModels(
            ...
            IEnumerable<DevicePropertyValueModel> devicePropertyValueModels)
        {
            var devicePropertyIndex = GetDevicePropertyConfiguration().ToDictionary(t => t.Name);

            var devicePropertiesType = deviceProperties.GetType();
            var propIndex = devicePropertiesType.GetProperties().ToDictionary(t => t.Name);

            var args = new object[1];
            foreach (var propVal in devicePropertyValueModels)
            {
                if ((propVal == null) ||
        ...
        private static IEnumerable<DevicePropertyValueModel> ExtractPropertyValueModels(DeviceProperties deviceProperties)
        {
            if (deviceProperties == null)
            {
                throw new ArgumentNullException(string.Format("deviceProperties is a null reference."));
            }

            var devicePropertyIndex = GetDevicePropertyConfiguration().ToDictionary(t => t.Name);

            // For now, display r/o properties first.
            var editableOrdering = 1;
            var nonediableOrdering = int.MinValue;
            ...
            foreach (PropertyInfo prop in devicePropertiesType.GetProperties())
            {
                ...
                var currentValue = getMethod.Invoke(deviceProperties, ReflectionHelper.EmptyArray);
        ...
        private async Task CheckIfDeviceExists(DeviceModel device, List<string> validationErrors)
        {
            ...
            {
                validationErrors.Add("Device already exists in the device registry.");
            }
        }
        ...
        private bool ValidateDeviceId(DeviceModel device, List<string> validationErrors)
        {
            ...
            {
                validationErrors.Add("DeviceID cannot be empty or null.");
                return false;
            }
